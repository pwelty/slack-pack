...
foreach ($channelMap as $tag=>$channel) {
  ...
  if ($specifiedTag!='' && $tag!=$specifiedTag) {
    $out .= "<p>Skipped ".$tag."</p>";
    continue;
  }
  echo "<p>Connecting to Pocket, looking for posts with tag = ".$tag."</p>";
...
function cmp($aChoice, $bChoice) {
  return $aChoice->time_added < $bChoice->time_added;
}
usort($thePosts, "cmp");
...
function postAPost($aPost) {
  global $slack,$pocket,$out,$pocketSuffix;
  // BUILD THE POST TEXT
